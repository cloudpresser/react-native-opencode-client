...
3. **Cached artifacts**: The `~/.gradle/caches` and `~/.gradle/wrapper` directories
   (Gradle build cache and distribution), plus the prebuild output in `android/`
   (generated by `expo prebuild`), are cached.

4. **Workflow behavior**:
   - Cache hit: EAS build reuses cached Gradle outputs. Gradle's up-to-date checks
     skip native recompilation. Only JS bundling + APK packaging runs (~3-4 min).
